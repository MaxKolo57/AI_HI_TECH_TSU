...
```mermaid
flowchart TB
    U["Пользователь"] --> A["Запуск run.sh → открытие GUI<br/>(«Ortho Segmentation — demo»)"]
    A -->|Ошибка запуска| Z["Ошибка: GUI не загружен"]

    A --> B["Загрузка TIFF-файла<br/>→ проверка формата и целостности"]
    B -->|Некорректный файл| Z1["Ошибка: недопустимый файл"]

    B --> C["Нарезка на тайлы 512×512<br/>→ подготовка данных для модели"]
    C --> D["Сегментация тайлов через SegFormer:<br/>• encoder: resnext101_32x16d<br/>• 10 классов<br/>• weights: 00100.pt<br/>• устройство: GPU/CPU"]
    D --> E["Реконструкция полной маски<br/>→ визуализация RGB-результата"]
    E --> F["Сохранение результата:<br/>экспорт mask.tif (GeoTIFF, 0–9, uint8)"]
